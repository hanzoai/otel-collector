...
func (cmd *bootstrap) CreateMigrationTables(ctx context.Context) error {
	...
	for _, migration := range schemamigrator.V2MigrationTablesLogs {
		for _, operation := range migration.UpItems {
			...
			err = cmd.migrationManager.RunOperation(ctx, operation, migration.MigrationID, schemamigrator.O11yLogsDB, true)
			if err != nil {
				return err
			}
		}
	...
	for _, migration := range schemamigrator.V2MigrationTablesMetrics {
		for _, operation := range migration.UpItems {
			...
			err = cmd.migrationManager.RunOperation(ctx, operation, migration.MigrationID, schemamigrator.O11yMetricsDB, true)
			if err != nil {
				return err
			}
		}
