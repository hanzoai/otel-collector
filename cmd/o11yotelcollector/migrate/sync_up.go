...
func registerSyncUp(parentCmd *cobra.Command, logger *zap.Logger) {
	syncUpCommand := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			up, err := newSyncUp(config.Datastore.DSN, config.Datastore.Cluster, config.Datastore.Replication, config.MigrateSyncUp.Timeout, logger)
			if err != nil {
				return err
			}
...
func (cmd *syncUp) SyncUp(ctx context.Context) error {
	...
	err = cmd.run(ctx, schemamigrator.TracesMigrations, schemamigrator.O11yTracesDB)
	if err != nil {
		return err
	}
	...
	err = cmd.run(ctx, logsMigrations, schemamigrator.O11yLogsDB)
	if err != nil {
		return err
	}

	err = cmd.run(ctx, schemamigrator.MetricsMigrations, schemamigrator.O11yMetricsDB)
	if err != nil {
		return err
	}

	err = cmd.run(ctx, schemamigrator.MetadataMigrations, schemamigrator.O11yMetadataDB)
	if err != nil {
		return err
	}

	err = cmd.run(ctx, schemamigrator.AnalyticsMigrations, schemamigrator.O11yAnalyticsDB)
	if err != nil {
		return err
	}

	err = cmd.run(ctx, schemamigrator.MeterMigrations, schemamigrator.O11yMeterDB)
	if err != nil {
		return err
	}
...
func (cmd *syncUp) runSquashedMigrations(ctx context.Context) error {
	squashedMigrations := map[string][]schemamigrator.SchemaMigrationRecord{
		schemamigrator.O11yLogsDB:    schemamigrator.CustomRetentionLogsMigrations,
		schemamigrator.O11yMetricsDB: schemamigrator.SquashedMetricsMigrations,
		schemamigrator.O11yTracesDB:  schemamigrator.SquashedTracesMigrations,
	}
