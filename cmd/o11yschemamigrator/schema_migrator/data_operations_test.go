...
func TestInsertIntoTable(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			op: InsertIntoTable{
				Database: constants.O11yMetadataDB,
			...
			want: fmt.Sprintf("INSERT INTO %s.distributed_column_evolution_metadata (signal, column_name, column_type, field_context, field_name, version, release_time) VALUES ('logs', 'body_promoted', 'JSON', 'body', 'message', 0, %d)", constants.O11yMetadataDB, timestamp),
		},
	}
