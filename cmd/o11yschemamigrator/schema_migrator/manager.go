...
var (
	...
	O11yLogsDB          = "o11y_logs"
	O11yMetricsDB       = "o11y_metrics"
	O11yTracesDB        = "o11y_traces"
	O11yMetadataDB      = "o11y_metadata"
	O11yAnalyticsDB     = "o11y_analytics"
	O11yMeterDB         = "o11y_meter"
	Databases             = []string{O11yTracesDB, O11yMetricsDB, O11yLogsDB, O11yMetadataDB, O11yAnalyticsDB, O11yMeterDB}
...
func (m *MigrationManager) Bootstrap() error {
	...
	for _, migration := range V2MigrationTablesLogs {
		for _, item := range migration.UpItems {
			if err := m.RunOperation(context.Background(), item, migration.MigrationID, O11yLogsDB, true); err != nil {
	...
	for _, migration := range V2MigrationTablesMetrics {
		for _, item := range migration.UpItems {
			if err := m.RunOperation(context.Background(), item, migration.MigrationID, O11yMetricsDB, true); err != nil {
				return errors.Join(ErrFailedToCreateSchemaMigrationsV2, err)
			}
		}
	}
	...
	for _, migration := range V2MigrationTablesTraces {
		for _, item := range migration.UpItems {
			if err := m.RunOperation(context.Background(), item, migration.MigrationID, O11yTracesDB, true); err != nil {
				return errors.Join(ErrFailedToCreateSchemaMigrationsV2, err)
			}
		}
	}
	...
	for _, migration := range V2MigrationTablesMetadata {
		for _, item := range migration.UpItems {
			if err := m.RunOperation(context.Background(), item, migration.MigrationID, O11yMetadataDB, true); err != nil {
				return errors.Join(ErrFailedToCreateSchemaMigrationsV2, err)
			}
		}
	}
	...
	for _, migration := range V2MigrationTablesAnalytics {
		for _, item := range migration.UpItems {
			if err := m.RunOperation(context.Background(), item, migration.MigrationID, O11yAnalyticsDB, true); err != nil {
				return errors.Join(ErrFailedToCreateSchemaMigrationsV2, err)
			}
		}
	}
	...
	for _, migration := range V2MigrationTablesMeter {
		for _, item := range migration.UpItems {
			if err := m.RunOperation(context.Background(), item, migration.MigrationID, O11yMeterDB, true); err != nil {
				return errors.Join(ErrFailedToCreateSchemaMigrationsV2, err)
			}
		}
	}
...
func (m *MigrationManager) runCustomRetentionMigrationsForLogs(ctx context.Context) error {
	...
	should, err := m.shouldRunSquashed(ctx, "o11y_logs")
	if err != nil {
		return err
	}
	...
	for _, migration := range CustomRetentionLogsMigrations {
		for _, item := range migration.UpItems {
			if err := m.RunOperation(ctx, item, migration.MigrationID, "o11y_logs", false); err != nil {
				return err
			}
		}
	}
...
func (m *MigrationManager) runSquashedMigrationsForMetrics(ctx context.Context) error {
	...
	should, err := m.shouldRunSquashed(ctx, O11yMetricsDB)
	if err != nil {
		return err
	}
	if !should {
	...
	for _, migration := range SquashedMetricsMigrations {
		for _, item := range migration.UpItems {
			if err := m.RunOperation(ctx, item, migration.MigrationID, O11yMetricsDB, false); err != nil {
				return err
			}
		}
	}
...
func (m *MigrationManager) runSquashedMigrationsForTraces(ctx context.Context) error {
	...
	should, err := m.shouldRunSquashed(ctx, O11yTracesDB)
	if err != nil {
		return err
	}
	if !should {
	...
	for _, migration := range SquashedTracesMigrations {
		for _, item := range migration.UpItems {
			if err := m.RunOperation(ctx, item, migration.MigrationID, O11yTracesDB, false); err != nil {
				return err
			}
		}
	}
...
func (m *MigrationManager) MigrateUpSync(ctx context.Context, upVersions []uint64) error {
	...
	for _, migration := range TracesMigrations {
		if !m.shouldRunMigration(O11yTracesDB, migration.MigrationID, upVersions) {
			continue
		}
		if err := m.executeSyncOperations(ctx, migration.UpItems, migration.MigrationID, O11yTracesDB); err != nil {
			return err
		}
	}
	...
	for _, migration := range logsMigrations {
		if !m.shouldRunMigration(O11yLogsDB, migration.MigrationID, upVersions) {
			continue
		}
		if err := m.executeSyncOperations(ctx, migration.UpItems, migration.MigrationID, O11yLogsDB); err != nil {
			return err
		}
	}

	for _, migration := range MetricsMigrations {
		if !m.shouldRunMigration(O11yMetricsDB, migration.MigrationID, upVersions) {
			continue
		}
		if err := m.executeSyncOperations(ctx, migration.UpItems, migration.MigrationID, O11yMetricsDB); err != nil {
			return err
		}
	}

	for _, migration := range MetadataMigrations {
		if !m.shouldRunMigration(O11yMetadataDB, migration.MigrationID, upVersions) {
			continue
		}
		if err := m.executeSyncOperations(ctx, migration.UpItems, migration.MigrationID, O11yMetadataDB); err != nil {
			return err
		}
	}

	for _, migration := range AnalyticsMigrations {
		if !m.shouldRunMigration(O11yAnalyticsDB, migration.MigrationID, upVersions) {
			continue
		}
		if err := m.executeSyncOperations(ctx, migration.UpItems, migration.MigrationID, O11yAnalyticsDB); err != nil {
			return err
		}
	}

	for _, migration := range MeterMigrations {
		if !m.shouldRunMigration(O11yMeterDB, migration.MigrationID, upVersions) {
			continue
		}
		if err := m.executeSyncOperations(ctx, migration.UpItems, migration.MigrationID, O11yMeterDB); err != nil {
			return err
		}
	}
...
func (m *MigrationManager) MigrateDownSync(ctx context.Context, downVersions []uint64) error {
	...
	for _, migration := range TracesMigrations {
		if !m.shouldRunMigration(O11yTracesDB, migration.MigrationID, downVersions) {
			continue
		}
		if err := m.executeSyncOperations(ctx, migration.DownItems, migration.MigrationID, O11yTracesDB); err != nil {
			return err
		}
	}
	...
	for _, migration := range logsMigrations {
		if !m.shouldRunMigration(O11yLogsDB, migration.MigrationID, downVersions) {
			continue
		}
		if err := m.executeSyncOperations(ctx, migration.DownItems, migration.MigrationID, O11yLogsDB); err != nil {
			return err
		}
	}

	for _, migration := range MetricsMigrations {
		if !m.shouldRunMigration(O11yMetricsDB, migration.MigrationID, downVersions) {
			continue
		}
		if err := m.executeSyncOperations(ctx, migration.DownItems, migration.MigrationID, O11yMetricsDB); err != nil {
			return err
		}
	}

	for _, migration := range AnalyticsMigrations {
		if !m.shouldRunMigration(O11yAnalyticsDB, migration.MigrationID, downVersions) {
			continue
		}
		if err := m.executeSyncOperations(ctx, migration.DownItems, migration.MigrationID, O11yAnalyticsDB); err != nil {
			return err
		}
	}

	for _, migration := range MeterMigrations {
		if !m.shouldRunMigration(O11yMeterDB, migration.MigrationID, downVersions) {
			continue
		}
		if err := m.executeSyncOperations(ctx, migration.DownItems, migration.MigrationID, O11yMeterDB); err != nil {
			return err
		}
	}
...
func (m *MigrationManager) MigrateUpAsync(ctx context.Context, upVersions []uint64) error {
	...
	for _, migration := range logsMigrations {
		...
		for _, item := range migration.UpItems {
			...
			if item.IsIdempotent() {
				if err := m.RunOperation(ctx, item, migration.MigrationID, O11yLogsDB, false); err != nil {
					return err
				}
			}
		}
