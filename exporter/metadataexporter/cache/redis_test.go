...
func buildMockRedisKeyCache(_ *testing.T, mockFn func(kvmock.ClientMock)) (*RedisKeyCache, kvmock.ClientMock) {
...
func TestRedisKeyCache_AddAttrsToResource_NewResource_Success(t *testing.T) {
	...
	cache, mock := buildMockRedisKeyCache(t, func(m kvmock.ClientMock) {
...
func TestRedisKeyCache_CardinalityLimitExceededMulti(t *testing.T) {
	ctx := context.Background()
	epochWindow := getCurrentEpochWindowMillis()
	fullKey := fmt.Sprintf("testTenant:metadata:traces:%d:resource:1", epochWindow)
	partialKey := fmt.Sprintf("testTenant:metadata:traces:%d:resource:2", epochWindow)

	cache, mock := buildMockRedisKeyCache(t, func(m kvmock.ClientMock) {
		// Limit is 3 for traces: resource 1 is full, resource 2 is not.
		m.ExpectSCard(fullKey).SetVal(3)
		m.ExpectSCard(partialKey).SetVal(1)
	})

	out, err := cache.CardinalityLimitExceededMulti(ctx, []uint64{1, 2}, pipeline.SignalTraces)
	require.NoError(t, err)
