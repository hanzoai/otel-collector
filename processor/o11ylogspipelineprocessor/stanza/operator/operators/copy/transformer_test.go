...
func TestBuildAndProcess(t *testing.T) {
	...
	cases := []testCase{
		...
		},
		{
			...
			func() *Config {
				cfg := NewConfig()
				cfg.From = o11ystanzaentry.Field{FieldInterface: o11ystanzaentry.NewBodyField("nested", "nestedkey")}
				cfg.To = entry.NewBodyField("key2")
				return cfg
			}(),
			newTestEntry,
		...
		},
		{
			...
			func() *Config {
				cfg := NewConfig()
				cfg.From = o11ystanzaentry.Field{FieldInterface: o11ystanzaentry.NewBodyField()}
		...
		},
		{
			...
			func() *Config {
				cfg := NewConfig()
				cfg.From = o11ystanzaentry.Field{FieldInterface: entry.NewAttributeField("nonexistentkey")}
				cfg.To = entry.NewResourceField("key2")
				return cfg
			}(),
			newTestEntry,
