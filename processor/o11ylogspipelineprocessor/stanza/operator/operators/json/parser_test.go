...
func TestParser(t *testing.T) {
	cases := []struct {
		...
		},
		{
			"with_timestamp",
			func(p *Config) {
				parseFrom := o11ystanzaentry.Field{FieldInterface: entry.NewAttributeField("timestamp")}
				p.TimeParser = &o11ystanzahelper.TimeParser{
					ParseFrom:  &parseFrom,
					LayoutType: "epoch",
					Layout:     "s",
				}
		...
		},
		{
			"with_scope",
			func(p *Config) {
				p.ScopeNameParser = &o11ystanzahelper.ScopeNameParser{
					ParseFrom: o11ystanzaentry.Field{FieldInterface: entry.NewAttributeField("logger_name")},
				}
			},
			&entry.Entry{
