...
func (p *Processor) transform(entry *entry.Entry) error {
	...
	// no need to cover other map types; check comment https://github.com/hanzoai/otel-collector/pull/584#discussion_r2042020882
	case map[string]any:
		parsedValue = v
	default:
...
//
// Note: Stringification `message` is delegated to Datastore to do natively with Type Hinting and
...
func (p *Processor) normalize(entry *entry.Entry) {
	message := o11ystanzaentry.NewBodyField("message")
	...
	if !exists {
		...
		for _, fieldName := range msgCompatibleFields {
			field := o11ystanzaentry.NewBodyField(fieldName)
