...
type processorImp struct {
	...
	dimensions             []dimension // o11y_latency metric
	expDimensions          []dimension // o11y_latency exphisto metric
	callDimensions         []dimension // o11y_calls_total metric
	dbCallDimensions       []dimension // o11y_db_latency_* metric
	externalCallDimensions []dimension // o11y_external_call_latency_* metric

	// The starting time of the data points.
	startTimestamp pcommon.Timestamp

	// Histogram.
	histograms    map[metricKey]*histogramData // o11y_latency metric
	latencyBounds []float64

	expHistograms map[metricKey]*exponentialHistogram

	callHistograms    map[metricKey]*histogramData // o11y_calls_total metric
	callLatencyBounds []float64

	dbCallHistograms    map[metricKey]*histogramData // o11y_db_latency_* metric
	dbCallLatencyBounds []float64

	externalCallHistograms    map[metricKey]*histogramData // o11y_external_call_latency_* metric
...
func newProcessor(logger *zap.Logger, instanceID string, config component.Config, ticker *clock.Ticker) (*processorImp, error) {
	logger.Info("Building o11yspanmetricsprocessor with config", zap.Any("config", config))
...
func (p *processorImp) Start(ctx context.Context, host component.Host) error {
	p.logger.Info("Starting o11yspanmetricsprocessor with config", zap.Any("config", p.config))
	...
	for k, exp := range exporters[pipeline.SignalMetrics] {
		...
		p.logger.Debug("Looking for o11yspanmetrics exporter from available exporters",
			zap.String("o11yspanmetrics-exporter", p.config.MetricsExporter),
			zap.Any("available-exporters", availableMetricsExporters),
		)
		for _, name := range exporterNames {
			if k.String() == name {
				p.metricsConsumer = append(p.metricsConsumer, metricsExp)
				p.logger.Info("Found exporter", zap.String("o11yspanmetrics-exporter", name))
			}
		}
	}
	...
	p.logger.Info("Started o11yspanmetricsprocessor")
...
func (p *processorImp) Shutdown(ctx context.Context) error {
	p.logger.Info("Shutting down o11yspanmetricsprocessor")
...
func (p *processorImp) buildMetrics() (pmetric.Metrics, error) {
	...
	ilm.Scope().SetName("o11yspanmetricsprocessor")
...
func (p *processorImp) collectExpHistogramMetrics(ilm pmetric.ScopeMetrics) error {
	...
	mExpLatency.SetName("o11y_latency")
...
func (p *processorImp) collectDBCallMetrics(ilm pmetric.ScopeMetrics) error {
	...
	mDBCallSum.SetName("o11y_db_latency_sum")
	...
	mDBCallCount.SetName("o11y_db_latency_count")
...
func (p *processorImp) collectExternalCallMetrics(ilm pmetric.ScopeMetrics) error {
	...
	mExternalCallSum.SetName("o11y_external_call_latency_sum")
	...
	mExternalCallCount.SetName("o11y_external_call_latency_count")
...
func (p *processorImp) collectCallMetrics(ilm pmetric.ScopeMetrics) error {
	...
	mCalls.SetName("o11y_calls_total")
...
func (p *processorImp) aggregateMetrics(traces ptrace.Traces) {
	for i := 0; i < traces.ResourceSpans().Len(); i++ {
		...
		resourceAttr.PutStr(o11yID, p.instanceID)
