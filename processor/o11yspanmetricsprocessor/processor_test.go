...
import (
	...
	"github.com/hanzoai/otel-collector/processor/o11yspanmetricsprocessor/mocks"
)
...
func verifyConsumeMetricsInput(t testing.TB, input pmetric.Metrics, expectedTemporality pmetric.AggregationTemporality, numCumulativeConsumptions int) bool {
	...
	assert.Equal(t, "o11yspanmetricsprocessor", ilm.At(0).Scope().Name())

	m := ilm.At(0).Metrics()
	// 6 metrics: o11y_calls_total, o11y_latency, o11y_db_latency_sum
	// o11y_db_latency_count, o11y_external_latency_sum, o11y_external_latency_count
	require.Equal(t, 6, m.Len())

	seenMetricIDs := make(map[metricID]bool)
	// The first 3 data points are for call counts.
	assert.Equal(t, "o11y_calls_total", m.At(0).Name())
	...
	assert.Equal(t, "o11y_latency", m.At(1).Name())
